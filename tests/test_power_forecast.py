...
def test_all_scheduled_on():
    ...
    print(df_schedules[f"schedule {devices_list[0].device_name}"].to_list() == [1, 1, 1])
    assert df_schedules[f"schedule {devices_list[0].device_name}"].to_list() == [1, 1, 1]
...
def test_schedule_first_device_on_first_half_second_device_second_half():
    ...
    df_predicted_power = pd.DataFrame({
        'power_kw': [0.5, 0.5, 1, 1]
    }, index=datetimes)
    devices_list = [socket_device("", "HWE-SKT", "test socket low power", 500, 250, 2, False), socket_device("", "HWE-SKT", "test socket high power", 1000, 500, 1, True)]
    df_schedules = schedule_devices(df_predicted_power, devices_list, 900)
    assert ((df_schedules[f"schedule {devices_list[0].device_name}"].to_list() == [1, 1, 0, 0]) and 
            (df_schedules[f"schedule {devices_list[1].device_name}"].to_list() == [0, 0, 1, 1]))
...
def test_schedule_second_device_only():
    ...
    devices_list = [socket_device("", "HWE-SKT", "test socket low power", 500, 250, 2, False), socket_device("", "HWE-SKT", "test socket high power", 1000, 1000, 1, True)]
    df_schedules = schedule_devices(df_predicted_power, devices_list, 900)
    assert ((df_schedules[f"schedule {devices_list[0].device_name}"].to_list() == [0, 0, 0, 0]) and 
            (df_schedules[f"schedule {devices_list[1].device_name}"].to_list() == [1, 1, 1, 1]))
