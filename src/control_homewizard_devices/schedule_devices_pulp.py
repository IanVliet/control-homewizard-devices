...
class DeviceSchedulingOptimizationPULP:
    ...
    def create_variables(self, number_timesteps: int):
        ...
        schedules = {
            (device.device_name, t): LpVariable(
                ...
                cat=device.policy.schedule_variable_cat.to_pulp(),
            )
    ...
    def solve_schedule_devices(
        ...
        results: list[ScheduleResultPULP] = []
        ...
        diff = {
            (device.device_name, t): LpVariable(
                ...
                cat=device.policy.diff_variable_cat.to_pulp(),
            )
            for device in self.socket_list
            for t in range(number_timesteps - 1)
        }
    ...
    def print_results(self, results: list[ScheduleResultPULP]):
        """
